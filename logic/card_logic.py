...
class CardLogic:
    ...
    def get_defence_from(self, offenciveCard: Card) -> Optional[int]:
        ...
        return first_or_none(
            [
                defence.value
                for defence in self.model.defensiveFrom
                if defence.offence == offenciveCard
            ]
        )
    ...
    def to_ui(self, extended=False) -> UiCard:
        ...
        return UiCard(
            ...
            type=card_type,
            off_against=None
            if card_type == UiCardType.Defence
            else [
                CardPairing(o.defenceCardId, o.value)
                for o in self.model.offensiveAgainst
            ],
            def_against=None
            if card_type != UiCardType.Defence
            else [
