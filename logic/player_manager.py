...
class PlayerManager:
    ...
    def create_player(self, name: str, game: "GameLogic") -> PlayerLogic:
        player = Player(
            name=name, game=game.model, money=0, isAdmin=False, isOnline=False
        )
        ...
        try:
            ...
            raise UserError(
                "Игрок с таким именем уже существует. Пожалуйста, измените имя.",
    ...
    def delete_player(self, player: PlayerLogic):
        try:
            ...
            self.db.session.rollback()
            raise UserError(
                "Не удалось удалить игрока из игры. По идее, вы не должны видеть эту ошибку",
