...
class Defence(db.Model):
    ...
    defence = db.relationship(
        "Card",
        back_populates="defensiveFrom",
        foreign_keys=[defenceCardId],
        uselist=False,
    )
    offence = db.relationship(
        "Card",
        back_populates="offensiveAgainst",
        foreign_keys=[offenceCardId],
        uselist=False,
    )
