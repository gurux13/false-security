...
class Game(db.Model):
    ...
    players = db.relationship(
        "Player", back_populates="game", cascade="all,delete", order_by="asc(Player.id)"
    )
    deck = db.relationship(
        "DeckEntry", back_populates="game", cascade="all,delete", lazy=True
    )
    discard = db.relationship(
        "DiscardEntry", back_populates="game", cascade="all,delete", lazy=True
    )
    rounds = db.relationship(
        "GameRound",
        back_populates="game",
        cascade="all,delete",
        order_by="desc(GameRound.roundNo)",
    )
