...
def create_offence_type(data: list):
    cardTypeOffence = CardType(
        name="Нападение", color="Red", enumType=CardTypeEnum.OFFENCE
    )
...
def create_accident_type(data: list):
    cardTypeAccident = CardType(
        name="Случайность", color="Blue", enumType=CardTypeEnum.ACCIDENT
    )
...
def create_defence_type(data: list):
    cardTypeDefence = CardType(
        name="Защита", color="Green", enumType=CardTypeEnum.DEFENCE
    )
